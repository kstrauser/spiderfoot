    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if rec.get("os") is not None:
                ...
                evt = SpiderFootEvent(
                    "OPERATING_SYSTEM", rec.get("os") + " (" + addr + ")", self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            if rec.get("devtype") is not None:
                # Notify other modules of what you've found
                evt = SpiderFootEvent(
                    "DEVICE_TYPE", rec.get("devtype") + " (" + addr + ")", self.__name__, event
                )
                self.notifyListeners(evt)
                ...
                for r in rec["data"]:
                    ...
                    if vulns is not None:
                        for vuln in list(vulns.keys()):
                            evt = SpiderFootEvent("VULNERABILITY", vuln, self.__name__, event)
                            self.notifyListeners(evt)
